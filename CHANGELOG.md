...
- **BREAKING (exception type change): column-not-found errors now raise `ColumnNotFoundError` instead of a plain `ValueError`** at three call sites that previously did an ad-hoc `raise ValueError(f"...not found in dataframe")`, plus `utils.validate_columns(..., raise_on_missing=True)`. `ColumnNotFoundError` subclasses `ValidationError` -> `MLToolkitError` -> `Exception`, **not** `ValueError`, so `except ValueError:` around these calls will no longer catch it - callers must switch to `except ColumnNotFoundError:` (or the `MLToolkitError`/`ValidationError` base). This is part of wiring the previously-dead `ColumnNotFoundError` from the public exception hierarchy (`feature_engineering_tk/exceptions.py`) into real call sites so `except ColumnNotFoundError:` actually catches something. Affected sites:
  - `utils.validate_columns(df, columns, raise_on_missing=True)` - now raises `ColumnNotFoundError` for the first missing column, with `available_columns` populated from the DataFrame.
  - `FeatureSelector.select_by_target_correlation()`, `.select_by_statistical_test()`, `.select_by_importance()` - target-column-not-found check.
  - `TargetAnalyzer.__init__()` - target-column-not-found check (the invalid-`task` check is unaffected and still raises `ValueError`).
...
- **`TargetAnalyzer.generate_full_report()` / `export_report()` now default to the fullest available analysis** - `generate_full_report()` gained `check_assumptions`, `report_effect_sizes`, `correct_multiple_tests`, and `alpha` parameters (all defaulting to the assumption-checking / effect-size / FDR-corrected behavior, i.e. `True`/`0.05`), threaded into its internal `analyze_feature_target_relationship()` call. `export_report()` now accepts `**report_kwargs` and forwards them to `generate_full_report()`. Existing no-arg calls to either method still work, but the `relationships` section of the report is now richer by default: `pvalue_corrected` / `significant_corrected` replace the plain `significant` column (multiple-testing correction is on by default), and `effect_size` / `effect_interpretation` / `assumptions_met` / `warnings` are included when applicable. Callers that parse `report['relationships']` items by the old `significant` key, or that snapshot-compare markdown/HTML report output, should pass `check_assumptions=False, report_effect_sizes=False, correct_multiple_tests=False` to `generate_full_report()`/`export_report()` to reproduce the exact pre-fix shape.

### Fixed

- **Wired 3 more previously-dead exceptions from `feature_engineering_tk/exceptions.py` into real raise sites** (following up on `ColumnNotFoundError` above - all four had been defined and exported from the public API since the exception hierarchy was introduced, but never actually raised anywhere):
  - `EmptyDataFrameError` - `FeatureEngineer.scale_features()` on a zero-row DataFrame used to crash with a low-level, confusing sklearn `ValueError` ("Found array with 0 sample(s)..."); it now raises `EmptyDataFrameError` before attempting to fit the scaler. (Note: `base.py`'s DataFrame-copy-at-init behavior is intentionally unchanged - initializing any toolkit class with an empty DataFrame still just logs a warning, per existing documented behavior.)
  - `InsufficientDataError` - `statistical_utils.apply_multiple_testing_correction([])` on an empty p-values list used to crash with a low-level `ZeroDivisionError` from inside `statsmodels.stats.multitest.multipletests`; it now raises `InsufficientDataError(operation='multiple testing correction', required=1, actual=0)` up front.
  - `ConstantColumnError` - `FeatureEngineer.create_binning(strategy='quantile')` on a constant (zero-variance) column used to silently succeed while producing an all-`NaN` binned column with a misleading "Binned column..." success log - `pandas.qcut()` collapses every bin edge to the same value and, with `duplicates='drop'`, returns all-`NaN` with no warning at all. It now raises `ConstantColumnError` up front. `strategy='uniform'` is unaffected (it produces a valid, if degenerate, single-bin result for constant data, so no guard was added there).
...
- **`generate_full_report()` never surfaced effect sizes / assumption checks / multiple-testing correction, even though the underlying `analyze_feature_target_relationship()` fully supports them** - `generate_full_report()` called it with no arguments, so a "comprehensive" report never included this data unless a caller separately called `analyze_feature_target_relationship(...)` themselves. See "Changed" above for the new parameters and the resulting shape change. The markdown and HTML exporters (`_export_markdown`/`_export_html`) now build the relationships table dynamically based on which optional columns (`pvalue_corrected`, `effect_size`, `warnings`) are actually present, instead of assuming the old fixed shape - so they render an "Effect Size" / "P-Value (Corrected)" column when available and degrade cleanly (no `KeyError`) when a caller opts out of one or more flags.
- **`generate_full_report()` redundantly recomputed `analyze_data_quality()`, `analyze_mutual_information()`, and `calculate_vif()`** - it called each once directly and then called `generate_recommendations()`, which (verified by reading the method bodies - none of `analyze_data_quality`/`analyze_mutual_information`/`calculate_vif` consult `self._analysis_cache`, which is currently only used by `analyze_class_distribution()`/`analyze_target_distribution()` in `core.py`) recomputed all three from scratch again internally. `generate_recommendations()` now accepts optional `quality`, `mi_df`, and `vif_df` arguments and only computes whichever of them isn't passed in; `generate_full_report()` computes each exactly once and passes its results through. Calling `generate_recommendations()` with no arguments (the existing public signature) is unchanged and still computes everything itself.
- **Bare `except:` clauses swallowed `KeyboardInterrupt`/`SystemExit` in addition to ordinary errors** - a bare `except:` in Python 3 catches `BaseException`, not just `Exception`, which is almost always unintended and can make a stuck operation un-interruptible. Narrowed to `except Exception as e:` with a `logger.debug(...)` call (so the resilient "don't crash the whole report/plot over one optional sub-computation" behavior is unchanged, but failures are now diagnosable) at three sites: the mutual-information and VIF recommendation blocks in `TargetAnalyzer.generate_recommendations()` (`quality.py`), and the `np.polyfit` regression-line-fitting block in `TargetAnalyzer.plot_feature_vs_target()` (`visualization.py`).
- **`statistical_utils.check_normality()` and `bootstrap_ci()` silently reseeded the global numpy random state** - `check_normality()`'s large-sample subsampling path called `np.random.seed(42)` (a hardcoded seed) before `np.random.choice(...)`, and `bootstrap_ci()` called `np.random.seed(random_state)` when a `random_state` was passed. Both are read-only statistical analysis functions, but as a side effect they permanently reset the caller's process-wide numpy RNG state - any code run afterward that depends on `np.random` (train/test splits, other bootstrapping, model initialization, etc.) would silently become deterministic/corrupted. Both now use a local `np.random.default_rng(...)` instance (`Generator.choice`) instead of the global `np.random.*` functions, so global state is left untouched; `bootstrap_ci(random_state=None)` now uses an unseeded local `Generator` rather than leaving the global generator's existing state in place, and `bootstrap_ci(random_state=<int>)` remains fully reproducible via the local `Generator`.
- **`calculate_correlation_ci()` returned a degenerate `[r, r]` confidence interval at `r = ±1` instead of `NaN`** - the Fisher Z-transformation is undefined at `r = ±1` (`arctanh(±1) = ±inf`), but because `np.tanh(±inf)` saturates back to `±1.0` instead of raising, the function silently produced a zero-width interval (e.g. `[1.0, 1.0]`) that reads as perfect statistical certainty rather than "this cannot be computed." Now returns `ci_lower`/`ci_upper` as `np.nan` for `abs(r) >= 1`, matching the existing `n < 4` branch's NaN-CI shape.
- **`cramers_v(correction=True)` docstring mischaracterized what `correction` does** - it was documented as "Apply bias correction for small samples," but the parameter is passed straight through to `scipy.stats.chi2_contingency(table, correction=correction)`, which applies Yates' continuity correction (only for 2x2 tables, a chi-square test-statistic continuity adjustment) - an unrelated concept from the general small-sample "bias-corrected Cramér's V" effect size (Bergsma & Wicher, 2013) the old wording implied. The docstring now accurately describes `correction` as Yates' continuity correction. Also added an opt-in `bias_correction: bool = False` parameter that, when `True`, computes the real Bergsma & Wicher bias-corrected Cramér's V and returns it as `cramers_v` (with the uncorrected value additionally available as `cramers_v_uncorrected`); default behavior/output shape is unchanged when `bias_correction` is omitted.
- **BEHAVIOR CHANGE: `remove_constant_columns()` and `validate_data_quality()` disagreed on what counts as "constant" when NaN is present** - `remove_constant_columns()` used `nunique() <= 1` (pandas' default `dropna=True`), while `validate_data_quality()`'s `constant_columns` check used `nunique(dropna=False) <= 1`. For a column like `[5, 5, NaN]`, `remove_constant_columns()` silently dropped it as constant while `validate_data_quality()` reported the dataframe as free of constant columns - a real, user-facing inconsistency between two methods describing the same concept. Standardized on `dropna=True` (matching pandas' `nunique()` default and `remove_constant_columns()`'s existing behavior): a column counts as constant if it has at most 1 distinct non-null value, so `[5, 5, NaN]` is constant (effectively single-valued, missingness aside) and an all-NaN column is also still correctly constant (`nunique() == 0`). **User-visible change**: `validate_data_quality()`'s `constant_columns` list (and the derived `issues_found` count) will now also flag columns that have exactly one non-null value alongside any amount of missing data - previously only columns constant across *all* rows including NaN (i.e. effectively only all-same or all-NaN columns) were flagged.
- **Mutable default argument in `clean_string_columns()`** - `operations: List[str] = ['strip', 'lower']` (flake8 B006) used a single list object as the default across all calls. Changed to `operations: Optional[List[str]] = None` with the default assigned inside the method body (`if operations is None: operations = ['strip', 'lower']`), matching the `Optional[...] = None` pattern used elsewhere in this file. No behavior change for existing callers.
...
- Added regression tests for the newly-wired `ColumnNotFoundError`/`EmptyDataFrameError`/`InsufficientDataError`/`ConstantColumnError` raise sites (`tests/test_utils.py`, `tests/test_feature_selection.py`, `tests/test_target_analyzer.py`, `tests/test_feature_engineering.py`, `tests/test_statistical_utils.py`), each covering both the new exception being raised with correct attributes/message and that existing valid-input behavior at the same call site is unaffected.
- Added tests for `generate_full_report()`'s new default-on parameters (effect sizes / corrected p-values present by default, explicit-off reproduces the legacy shape), `export_report()` kwarg-forwarding across all three formats, a call-count regression test proving `analyze_data_quality()`/`analyze_mutual_information()`/`calculate_vif()` each run exactly once per `generate_full_report()` call, a `generate_recommendations()` precomputed-arguments test, and `caplog`-based tests confirming the three narrowed `except Exception as e:` sites log a debug message (and still degrade gracefully) instead of swallowing failures silently.
- Added `TestGlobalRandomStateIsolation` (4 tests) proving `check_normality()` and `bootstrap_ci()` (both with and without `random_state`) leave `np.random.get_state()` unchanged, plus a reproducibility regression test confirming `bootstrap_ci(random_state=42)` still returns identical results across calls now that seeding goes through a local `Generator`.
- Added `r = ±1` NaN-CI regression tests for `calculate_correlation_ci()`, and `bias_correction` tests for `cramers_v()` (uncorrected-vs-corrected comparison, plus a small-table graceful-NaN case).
- Added tests proving `validate_data_quality()` flags a `[5, 5, NaN]`-style column and an all-NaN column as constant, plus a direct regression test asserting `remove_constant_columns()` and `validate_data_quality()` now agree on the same `[5, 5, NaN]` column. Added tests for `clean_string_columns()`'s default `operations` argument (omitted entirely, and confirming the default isn't a shared/mutated object across calls).
